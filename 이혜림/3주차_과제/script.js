...
async function fetchFacts(count = 1) {
  try {
    ...
    //  3. 로딩 상태 표시
    showLoading();
    const response = await fetch(url); // ✅ 4. API 요청을 보내고 응답을 기다림
    // → fetch()를 사용하여 네트워크 요청을 보낸다.

    if (!response.ok) {
      throw new Error("Failed tp fetch cat facts");
    ...
    showError(err.message); //  7. 에러 발생 시 사용자에게 알림
  } finally {
...
getMultipleBtn.addEventListener("click", async () => {
  ...
  if (facts && facts.length > 0) {
    displayFacts(facts); //  4. 응답 데이터가 있을 경우, 화면에 표시
  }
});
